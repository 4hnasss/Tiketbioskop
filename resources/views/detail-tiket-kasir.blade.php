...
<head>
    ...
    <title>Tiket {{ $tiket->kursi->nomorkursi }} - {{ $tiket->jadwal->film->judul }}</title>
...
<body class="bg-gray-50 min-h-screen py-8">
    ...
    <div class="no-print max-w-3xl mx-auto px-4 mb-6">
        <div class="flex justify-between items-center">
            <a href="{{ route('riwayat-tiket-kasir') }}" 
            ...
            <div class="grid grid-cols-2 divide-x-2 divide-dashed divide-gray-400">
                ...
                <div class="px-8 py-6 space-y-4">
                    ...
                    <div class="space-y-2 text-sm font-mono">
                        <div class="flex justify-between">
                            ...
                            <span class="font-bold text-gray-900">{{ \Carbon\Carbon::parse($tiket->jadwal->tanggal)->format('d-M-Y') }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="font-semibold text-gray-700">TIME</span>
                            <span class="font-bold text-gray-900">{{ \Carbon\Carbon::parse($tiket->jadwal->jamtayang)->format('H:i') }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="font-semibold text-gray-700">ROW</span>
                            <span class="font-bold text-gray-900">{{ substr($tiket->kursi->nomorkursi, 0, 1) }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="font-semibold text-gray-700">SEAT</span>
                            <span class="font-bold text-gray-900">{{ substr($tiket->kursi->nomorkursi, 1) }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="font-semibold text-gray-700">THEATRE</span>
                            <span class="font-bold text-gray-900">{{ $tiket->jadwal->studio->nama_studio }}</span>
                        </div>
                    </div>

                    <div class="pt-4 space-y-1 text-sm font-mono">
                        @php
                            $kursiList = is_array($tiket->transaksi->kursi) ? $tiket->transaksi->kursi : json_decode($tiket->transaksi->kursi, true);
                            $hargaPerKursi = $tiket->transaksi->totalharga / count($kursiList);
                        @endphp
                        <div class="flex justify-between">
                            <span class="font-bold text-gray-900">Rp {{ number_format($hargaPerKursi, 0, ',', '.') }}</span>
                            <span class="font-semibold text-gray-700">{{ strtoupper($tiket->transaksi->metode_pembayaran ?? 'CASH') }}</span>
                        </div>
                        <div>
                            <p class="text-xs font-bold text-gray-900">{{ $tiket->kodetiket }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-600">{{ $tiket->transaksi->user->name ?? 'GUEST' }}</p>
                            <p class="text-xs text-gray-600">INCL. TAX</p>
                            <p class="text-xs font-semibold text-gray-700">{{ $tiket->created_at->format('Y-m-d H:i:s') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="px-8 py-6 flex flex-col justify-between relative">
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="space-y-2 text-sm font-mono">
                            <div>
                                ...
                                <span class="font-bold text-gray-900 ml-2">{{ \Carbon\Carbon::parse($tiket->jadwal->tanggal)->format('D, d-M') }}</span>
                            </div>
                            <div>
                                <span class="font-semibold text-gray-700 mr-2">TIME</span>
                                <span class="text-gray-500">:</span>
                                <span class="font-bold text-gray-900 ml-2">{{ \Carbon\Carbon::parse($tiket->jadwal->jamtayang)->format('H:i') }}</span>
                            </div>
                            <div>
                                <span class="font-semibold text-gray-700 mr-2">ROW</span>
                                <span class="text-gray-500">:</span>
                                <span class="font-bold text-gray-900 ml-2">{{ substr($tiket->kursi->nomorkursi, 0, 1) }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="relative z-10 text-right">
                        ...
                        <p class="text-8xl font-black text-navy leading-none" style="color: #0A1D56;">{{ substr($tiket->kursi->nomorkursi, 1) }}</p>
                    </div>
                    ...
                    <div class="relative z-10 text-sm font-mono space-y-1">
                        ...
                        <div>
                            <span class="font-semibold text-gray-700 text-xs">{{ strtoupper($tiket->transaksi->metode_pembayaran ?? 'CASH') }}</span>
                        </div>
                        <div>
                            <p class="text-xs text-gray-600">INCL. TAX</p>
                            <p class="text-xs font-bold text-gray-900">{{ $tiket->kodetiket }}</p>
                            <p class="text-xs text-gray-600 mt-1">{{ $tiket->transaksi->user->name ?? 'GUEST' }}</p>
                        </div>
                    </div>
                </div>
        ...
        async function downloadTicket() {
            ...
            try {
                ...
                canvas.toBlob(function(blob) {
                    ...
                    const filmTitle = '{{ str_replace([" ", "/", "\\", ":", "*", "?", "\"", "<", ">", "|"], "-", $tiket->jadwal->film->judul) }}';
                    link.download = `Tiket-${filmTitle}-Kursi-{{ $tiket->kursi->nomorkursi }}-{{ $tiket->kodetiket }}.png`;
                    link.href = url;
